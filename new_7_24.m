...
mat_secs = windowMat(power_mats(:,:,1),'second',wind_size);
...
for i=1:length(mat_days)
    disp(i);
    utc_times(i) = datetime_to_unix(2013,mat_mons(i),mat_days(i),mat_hrs(i),mat_mins(i),mat_secs(i));
end

%%%Creating Occupancy ground truth vectors
%gotta change it to different wind size
...
for i=1:size(power_mats,3)
    pow_mats_mean(:,i) = windowMat(power_mats(:,:,i),'mean',wind_size);
    pow_mats_range(:,i) = windowMat(power_mats(:,:,i),'range',wind_size);
    pow_mats_peak(:,i) = windowMat(power_mats(:,:,i),'peak',wind_size);
end

disp('PF mats part');

for i=1:size(pf_mats,3)
    pf_mats_mean(:,i) = windowMat(pf_mats(:,:,i),'mean',wind_size);
    pf_mats_range(:,i) = windowMat(pf_mats(:,:,i),'range',wind_size);
    pf_mats_peak(:,i) = windowMat(pf_mats(:,:,i),'peak',wind_size);
end

disp('Light mats part');
...
if wind_size<1
    Light_mat = kron(vertcat(Light24,Light25,Light26,Light27,Light28,...
    Light29,Light30,Light31,Light1,Light2,Light3),ones(1/wind_size,1));
else
...
disp('Inter mini mats part');

inter_mini_mat = [utc_times circshift(pow_mats_mean,shift)...
    circshift(pow_mats_range,shift) circshift(pow_mats_peak,shift)...
    circshift(pf_mats_mean,shift) circshift(pf_mats_range,shift)...
    circshift(pf_mats_peak,shift) Light_mat mat_hrs occup_mat];
...
disp('Aggr and historical mats part');
...
pow_aggr_prev2 = pow_mats_aggr - circshift(pow_mats_aggr,2);
pf_aggr_prev1 = pf_mats_aggr - circshift(pf_mats_aggr,1);
pf_aggr_prev2 = pf_mats_aggr - circshift(pf_mats_aggr,2);

%%%Recombine to a big matrix 
disp('CSVmats part');

csv_mat = [utc_times pow_mats_aggr pf_mats_aggr ...
    pow_aggr_prev1 pow_aggr_prev2 pf_aggr_prev1 ...
    pf_aggr_prev2 Light_mat mat_hrs occup_mat];

csv_mat(1:3,:)=[];
csv_mat(csv_mat(:,end-1)==0,:)=[];
...
filename = 'CSVmat_10sec_prev21.arff';
...
fprintf(fid,['%s\n%s\n%s\n%s\n%s\n%s\n'],'@Attribute Aggr_pow_mean_prev1 Numeric', ...
     ...
     '@Attribute Aggr_pow_mean_prev2 Numeric','@Attribute Aggr_pow_range_prev2 Numeric',...
     '@Attribute Aggr_pow_peak_prev2 Numeric');
fprintf(fid,['%s\n%s\n%s\n%s\n%s\n%s\n'],'@Attribute Aggr_pf_mean_prev1 Numeric', ...
     '@Attribute Aggr_pf_range_prev1 Numeric','@Attribute Aggr_pf_peak_prev1 Numeric',...
     '@Attribute Aggr_pf_mean_prev2 Numeric','@Attribute Aggr_pf_range_prev2 Numeric',...
     '@Attribute Aggr_pf_peak_prev2 Numeric');
fprintf(fid,'%s\n','@Attribute light {0,1}');
...
figure(1);
hold on;
plot(csv_mat(:,1),circshift(csv_mat(:,2),-8*60));
