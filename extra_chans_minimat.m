...
real_mats_mean = inter_mini_mat(:,2:22);
real_mats_range = inter_mini_mat(:,23:43);
real_mats_peak = inter_mini_mat(:,44:64);
...
csv_mat = [utc_times real_mats_aggr reac_mats_aggr Light_mat mat_hrs occup_mat];
...
hold on;
plot(csv_mat(:,1),csv_mat(:,end-1),'r.-');
plot(csv_mat(:,1),csv_mat(:,end-9),'k.-');
%plot(csv_mat(:,1),csv_mat(:,end-5),'k.-');
hold off;



filename = 'CSVmat_w_shift_v2.arff';
fid = fopen(filename, 'w');
fprintf(fid,'%s\n\n','@RELATION Occupancy');
